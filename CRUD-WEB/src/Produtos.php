...
class Produtos
{
    ...
    public function uploadCsv(): void
    {
        if (isset($_FILES['csv_file']) && $_FILES['csv_file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (($handle = fopen(filename: $fileTmpPath, mode: "r")) !== false) {

                $query = "INSERT INTO produtos (nome, sku, quantidade, preco) VALUES (:nome, :sku, :quantidade, :preco)";
                $stmt = $this->pdo->prepare(query: $query);

                fgetcsv(stream: $handle, length: 1000, separator: ",");

                while (($data = fgetcsv(stream: $handle, length: 1000, separator: ",")) !== false) {
                    if (count(value: $data) < 4) {
                        continue;
                    }
